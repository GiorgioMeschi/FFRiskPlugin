...
class ProcessingHelper:
    ...
    def reproject_vector_layer(self, layer, prj):
        ...
        layer_res = processing.run('native:reprojectlayer', parameter)['OUTPUT'] 
    ...
    def max_sliding_windows(self, array, windows_size = (3,3)):
    ...
    def average_sliding_windows(self, array, windows_size = 3):
        ...
        filtered_array = uniform_filter(array, size=windows_size, mode='constant')
    ...
    def merge_vector_layers(self, list_poi_vector_layers, crs, out_file, complete_out_path):
        '''
        out_file is parameterAsOutputLayer()
        '''

        params = {
            'LAYERS': list_poi_vector_layers,
            'CRS': crs,
            'OUTPUT': out_file
        }
        ...
        if isinstance(out_shp_path, QgsVectorLayer):    
            print('change merged file to temp layer')
            # out_shp_path = QgsProcessingUtils.generateTempFilename(complete_out_path)
            
            out_shp_file = QTemporaryFile(complete_out_path)
        
            # Open the file and get the file name
            out_shp_file.open()
            out_shp_path = out_shp_file.fileName().split('.')[0] + '.shp'
            out_shp_file.close()
            print(out_shp_path)
